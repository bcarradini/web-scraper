...
HeadlessRenderer.prototype.render = function(scrapeUrl, actions, cookiejar) {
  ...
  log.error('TEMP: HeadlessRenderer.prototype.render: 1');
  var spooky = new Spooky(settings, function() {
    log.error('TEMP: HeadlessRenderer.prototype.render: 2');
    ...
    log.error('TEMP: HeadlessRenderer.prototype.render: 3');

    spooky.run();
  });

  log.error('TEMP: HeadlessRenderer.prototype.render: 4');

  spooky.on('pageDownload', function(body) {
    log.error('TEMP: spooky.on pageDownload: 5');
    renderer.emit('renderer.urlRendered', scrapeUrl, body);
    log.error('TEMP: spooky.on pageDownload: 6');
  });
...
HeadlessRenderer.prototype.settings = function(loglevel) {
  ...
  log.error('TEMP: HeadlessRenderer.prototype.settings: PHANTOMJS_EXECUTABLE: '+env['PHANTOMJS_EXECUTABLE']);  
  return {
    child: {
