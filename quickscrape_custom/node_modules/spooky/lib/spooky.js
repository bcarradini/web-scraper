...
Spooky.prototype._spawnChild = function () {
    log.error('TEMP: _spawnChild: begin');
    ...
    log.error('TEMP: _spawnChild: '+options.command+' '+args);
    child = this._child = spawn(options.command, args, options.spawnOptions);

    // TEMP
    child.on('error', (err) => {
      console.log('TEMP: Failed to start child.');
    });
    // TEMP
